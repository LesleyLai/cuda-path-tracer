...
TEST_CASE("Transforming Ray")
{
  ...
  {
    const auto transform = glm::translate(glm::mat4{1.0}, glm::vec3(1, 1, 1));
    const auto transformed = transform_ray(transform, ray);
    REQUIRE_THAT(transformed.origin, ApproxEqual(glm::vec3(2, 3, 4)));
  ...
  SECTION("Scaling ray has no effect")
  {
    const auto transform = glm::scale(glm::mat4{1.0}, glm::vec3(2, 2, 2));
    const auto transformed = transform_ray(transform, ray);
    REQUIRE_THAT(transformed.origin, ApproxEqual(ray.origin));
    REQUIRE_THAT(transformed.direction, ApproxEqual(ray.direction));
    REQUIRE(transformed.t_min == ray.t_min);
    REQUIRE(transformed.t_max == ray.t_max);
  }

  SECTION("Rotating ray rotates the direction")
  {
